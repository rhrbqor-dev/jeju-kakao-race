...
6. 관리자 페이지의 `참가 기능/랭킹 설정 > 이미지 보안전송 블록 ID`에 현재 행사 봇의 **사진 인증 일반 블록 ID**를 저장합니다. 비즈플러그인 ID를 넣으면 안 됩니다. 여러 행사·봇을 운영할 때는 행사마다 각각 저장합니다. 모든 행사에서 같은 블록을 쓰는 경우에만 Render 환경변수 `KAKAO_SECURE_IMAGE_BLOCK_ID`를 공통 기본값으로 사용할 수 있습니다.
...
행사별 이미지 보안전송 블록 ID 또는 `KAKAO_SECURE_IMAGE_BLOCK_ID`가 등록되면 사진 업로드 버튼이 해당 블록을 직접 호출하므로 `사진 인증` 발화의 인식 여부와 관계없이 플러그인이 열립니다. 둘 다 없으면 기존 메시지 발화 방식으로 동작합니다.
